...
A proof-of-concept for unit testing Redshift SQL (`.rsql`) scripts using **pytest**, targeting Python-based data pipelines that execute raw SQL files against Amazon Redshift.
...
This POC demonstrates how to test these `.rsql` files locally using a PostgreSQL-compatible database as a Redshift substitute, with [sqlglot](https://github.com/tobymao/sqlglot) handling the dialect translation.

## What This Tests

This POC tests **SQL logic correctness**: do your JOINs, aggregations, CASE expressions, and data transformations produce the right rows? It does this by transpiling Redshift SQL to PostgreSQL and running it against a local database.
...
This POC supports two database backends, mirroring the Java POC's H2 vs Testcontainers pattern:

| Backend | Command | Requires | Speed |
|---|---|---|---|
| **PGlite** (default) | `pytest` | Node.js only | ~34s |
| **Testcontainers** | `pytest --db-backend=testcontainers` | Docker or Podman | ~60s |

**PGlite** runs an embedded PostgreSQL instance via [PGlite](https://pglite.dev/) (a WASM build of Postgres running in Node.js). No Docker, no containers, no external database — just Node.js. This is the recommended backend for local development.

**Testcontainers** starts a real PostgreSQL 16 container for full-fidelity testing. Use this in CI or when you need to validate against a real PostgreSQL server.

Both backends provide identical fixtures (`db_conn`, `db_conn_empty`, `sample_query_path`) so all tests run the same way regardless of backend.
...
```
pytest
  -> PGlite or Testcontainers starts a PostgreSQL instance
  -> For each test:
...
The key insight is that `rsql_executor.py` mirrors the production `execute_sql_scripts.py` flow, but replaces the `rsql` CLI (which requires a Redshift cluster) with `psycopg2` (which connects to a local PostgreSQL instance). sqlglot bridges the dialect differences.
...
├── .pglite/
│   ├── package.json                    # Node.js deps for PGlite backend
...
└── tests/
    ├── conftest.py                     # Test fixtures (PGlite/Testcontainers, DB setup)
...
- **Python 3.9+**
- **Node.js 18+** (required by the PGlite backend; v22 tested)
- **Docker or Podman** (only if using `--db-backend=testcontainers`)
...
### 2. Install PGlite Node.js dependencies (one time)
...
This creates `.pglite/node_modules/` with the PGlite packages. You only need to do this once after cloning (or after deleting `node_modules/`).

### 3. Run all tests (default: PGlite, no Docker needed)

```bash
pytest
```

### 4. Run tests with Testcontainers (requires Docker/Podman)

```bash
pytest --db-backend=testcontainers
```
...
The database instance is started **once per test session** (not per test) for performance. Each test gets a fresh database state via `DROP SCHEMA CASCADE` + recreation.

### PGlite backend notes

PGlite runs an embedded PostgreSQL via Node.js and only supports a single connection at a time. The test fixtures handle this transparently — one session-scoped connection is reused across all tests, with schema resets between tests providing isolation.
